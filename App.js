...
const App = () => {
  ...
  const [lifePlan, setLifePlan] = usePersistentState('nq_lifePlan', {
  ...
  const [goals, setGoals] = usePersistentState('nq_goals', []);

  const [tasks, setTasks] = usePersistentState('nq_tasks', []);

  const [reflections, setReflections] = usePersistentState('nq_reflections', []);
  ...
  const addReflection = (entry) => setReflections(prev => [{...entry, id: `r${Date.now()}`, date: new Date().toISOString() }, ...prev]);
  ...
  const renderView = () => {
    switch (view) {
      ...
      case 'goals':
        return <Goals goals={goals} onAddGoal={addGoal} onUpdateGoal={updateGoal} onDeleteGoal={deleteGoal} />;
      case 'action':
